                                     ...
                                     void *data OVS_UNUSED)
{
    ...
    SBREC_DATAPATH_BINDING_TABLE_FOR_EACH_TRACKED (dp, dp_table) {
        if (sbrec_datapath_binding_is_new(dp) ||
            ...
            /* The removal and addition is handler via the runtime_data. */
           return EN_HANDLED_UNCHANGED;
        }
...
#undef OVS_NODE
    ...
    // Port binding handler Advertise FDB for local VIF + Virtual port (options:virtual-ip) + GW routers and patch ports of connected DGP (local)
