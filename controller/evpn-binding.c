...
static uint32_t evpn_binding_hash(const struct in6_addr *remote_ip,
                                  uint32_t vni);
static struct evpn_binding * evpn_binding_find(
    const struct hmap *evpn_bindings, const struct in6_addr *remote_ip,
    uint32_t vni);
...
void
evpn_vtep_bindinga_list(struct unixctl_conn *conn, int argc OVS_UNUSED,
...
void
evpn_multicast_groups_list(struct unixctl_conn *conn, int argc OVS_UNUSED,
                           const char *argv[] OVS_UNUSED, void *data_)
{
