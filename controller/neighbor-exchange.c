...
static struct evpn_remote_vtep *evpn_remote_vtep_find(
    ...
    uint16_t port, uint32_t vni);
static void evpn_remote_vtep_format(struct ds *ds,
    ...
    VECTOR_FOR_EACH (n_ctx_in->monitored_interfaces, nim) {
        ...
        if (nim->type == NEIGH_IFACE_VXLAN) {
            ...
            VECTOR_FOR_EACH_PTR (&received_neighbors, ne) {
                ...
                if (!evpn_remote_vtep_find(n_ctx_out->remote_vteps, ne->addr,
                                           port, nim->vni)) {
                      ...
                      const char *argv[] OVS_UNUSED, void *data_)
{
    ...
    HMAP_FOR_EACH (vtep, hmap_node, remote_vteps) {
        evpn_remote_vtep_format(&ds, vtep);
        ds_put_char(&ds, '\n');
    }
                     ...
                     uint16_t port, uint32_t vni)
{
    ...
    *vtep = (struct evpn_remote_vtep) {
    ...
    if (VLOG_IS_DBG_ENABLED()) {
        struct ds ds = DS_EMPTY_INITIALIZER;
        evpn_remote_vtep_format(&ds, vtep);
        VLOG_DBG("Adding new remote VTEP: %s", ds_cstr_ro(&ds));
        ds_destroy(&ds);
    }
}
...
static void
evpn_remote_vtep_format(struct ds *ds, struct evpn_remote_vtep *vtep)
{
    ds_put_cstr(ds, "IP: ");
